...
class UserSeeder extends Seeder
{
    public function run(): void
    {
        ...
        DB::table('users')->insert([
            // Club 1 owner (admin)
            $userTemplate(SeederIds::$michal, 'Michal', 'Kašpařík', 'admin@teameo.cz'),
            // Coach 1 (head_coach U9+U12, also club admin)
            $userTemplate(SeederIds::$jan, 'Jan', 'Novák', 'trener@teameo.test'),
            // Coach 2 (assistant in club 1, head_coach in club 2)
            $userTemplate(SeederIds::$pavel, 'Pavel', 'Dvořák', 'trener2@teameo.test'),
            // Parent 1 — member in BOTH clubs (cross-club scenario)
            $userTemplate(SeederIds::$eva, 'Eva', 'Svobodová', 'rodic@teameo.test'),
            // Parent 2
            $userTemplate(SeederIds::$martin, 'Martin', 'Procházka', 'rodic2@teameo.test'),
            ...
            $userTemplate(SeederIds::$adam, 'Adam', 'Novotný', 'adam@teameo.test', [
                'birth_date' => '2010-11-03', 'is_minor' => true, 'can_self_manage' => true,
            ]),
            // Club 2 owner
            $userTemplate(SeederIds::$lucie, 'Lucie', 'Marková', 'lucie@teameo.test'),
            // Placeholder child (no guardian linked — unlinked scenario)
            $userTemplate(SeederIds::$petra, 'Petra', 'Nová', 'placeholder_petra@teameo.placeholder', [
                'password' => Hash::make(Str::random(64)),
                'status' => 'placeholder', 'birth_date' => '2017-09-22',
                'is_minor' => true, 'email_verified_at' => null,
            ]),
            // Adult athlete in club 2
            $userTemplate(SeederIds::$david, 'David', 'Kratochvíl', 'david@teameo.test'),
        ]);
        ...
        DB::table('users')->insert([
            $userTemplate(SeederIds::$hana, 'Hana', 'Králová', 'hana@teameo.test'),
            $userTemplate(SeederIds::$jiri, 'Jiří', 'Marek', 'jiri@teameo.test'),
        ]);
        ...
        $placeholderTemplate = fn ($first, $last, $birth) => $userTemplate(
            ...
            'placeholder_' . strtolower($first) . '@teameo.placeholder',
            [
                'password' => Hash::make(Str::random(64)),
                'status' => 'placeholder',
                'birth_date' => $birth,
