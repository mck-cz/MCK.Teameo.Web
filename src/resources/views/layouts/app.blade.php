...
<head>
    ...
    <title>@yield('title', config('app.name', 'Teameo'))</title>
...
<body class="bg-bg text-text text-sm font-sans min-h-screen flex flex-col antialiased">
    ...
    <div class="flex flex-1">
        ...
        <aside
            ...
            class="fixed inset-y-0 left-0 z-50 flex flex-col bg-primary-dark text-white transition-all duration-200 w-60 lg:static lg:z-auto"
        >
            <!-- Logo -->
            <div class="flex items-center h-16 px-4">
                <a href="/" class="flex items-center gap-2 overflow-hidden">
                    <div class="w-8 h-8 rounded-lg bg-white flex items-center justify-center shrink-0">
                        <span class="text-primary font-semibold text-base">T</span>
                    </div>
                    <span
                        x-data
                        :class="$store.sidebar.expanded ? 'lg:opacity-100 lg:w-auto' : 'lg:opacity-0 lg:w-0'"
                        class="font-semibold text-lg whitespace-nowrap transition-all duration-200 overflow-hidden"
                    >Teameo</span>
                </a>
            </div>
            ...
            @if($userClubs->count() > 1)
                <div x-data="{ clubOpen: false }" class="px-3 mb-2 relative">
                    <button
                        @click="clubOpen = !clubOpen"
                        class="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/10 text-white/80 hover:text-white transition-colors text-sm cursor-pointer"
                    >
                        ...
                        <span
                            x-data
                            :class="$store.sidebar.expanded ? 'lg:opacity-100 lg:w-auto' : 'lg:opacity-0 lg:w-0'"
                            class="truncate flex-1 text-left transition-all duration-200 overflow-hidden"
            ...
            @elseif($currentClub)
                <div x-data class="px-3 mb-2">
                    <div
                        class="flex items-center gap-2 px-2 py-1.5 text-white/60 text-sm"
                    >
                        <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                        </svg>
            ...
            <button
                ...
                class="hidden lg:flex items-center justify-center mx-3 mb-2 p-1.5 rounded-lg hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-white/30 transition-colors cursor-pointer"
            >
                <svg
            ...
            <nav class="flex-1 px-3 space-y-1 overflow-y-auto">
                ...
                @foreach($navItems as $item)
                    <a
                        href="{{ $item['href'] }}"
                        x-data
                        class="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors {{ request()->is(ltrim($item['href'], '/') . '*') || (request()->is('/') && $item['key'] === 'dashboard') ? 'bg-white/15 text-white' : 'text-white/70 hover:bg-white/10 hover:text-white' }}"
                    >
            ...
            <!-- Management section -->
            <div class="px-3 pt-2 pb-1 border-t border-white/10">
                @php
                    $mgmtItems = [
                ...
                @foreach($mgmtItems as $item)
                    <a
                        href="{{ $item['href'] }}"
                        x-data
                        class="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors text-sm {{ request()->is(ltrim($item['href'], '/') . '*') ? 'bg-white/15 text-white' : 'text-white/70 hover:bg-white/10 hover:text-white' }}"
                    >
                        <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="{{ $item['icon'] }}" />
                        </svg>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="lg:hidden flex items-center h-16 px-4 bg-surface border-b border-border">
                ...
                <span class="ml-3 font-semibold text-base">Teameo</span>
            </header>
